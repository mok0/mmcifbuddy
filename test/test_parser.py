...
def test_refine_ls_restr():
    assert 7 == len(thedata['_refine_ls_restr'])
...
def test_refine_ls_shell():
    assert 15 == len(thedata['_refine_ls_shell'])
...
def test_struct_conf():
    assert 20 == len(thedata['_struct_conf'])
    assert 14 == len(thedata['_struct_conf']['id'])

def test_struct_conn():
    assert 34 == len(thedata['_struct_conn'])
    assert 5 == len(thedata['_struct_conn']['id'])


def test_atom_site():
    assert 21 == len(thedata['_atom_site'])
